...
<div id="menu-container" class="ordering-menu-shell">
	<div class="ordering-menu-row">
		...
		<div id="listmenu" class="bg-black">
			<div class="foods menu-list-grid" id="listmenu_items" data-menu-list></div>
		</div>
		@include('ordering.partials.siderightbar')
	</div>
</div>
	...
	var mdb_ordering = function() {
		...
		async function initCateMenu() {
			...
			document.querySelector('#cate_list').innerHTML = cate.filter(f => f.menu.length > 0).map(function(f) {
				return `<a class="cate-card cate-link" href="#" idx="${ f.id }" role="button">
                            <span class="cate-card-icon">${f.icon??''}</span>
                            <span class="cate-card-label">${ f.name }</span>
                        </a>`
			}).join('')
			initCateLink()
		}
		...
		function takeOrder(id, data) {
			...
			aList.innerHTML = aItems.map((i, n) => {
				...
				el.querySelector('[item-name]').innerHTML = data[4]
		...
		function setActiveCateLink(el) {
			document.querySelectorAll('.cate-link.is-active').forEach(function(n) {
		...
		function initCateLink() {
			...
			var firstCate = document.querySelector('#cate_list .cate-link')
			if (firstCate && !document.querySelector('#cate_list .cate-link.is-active')) {
